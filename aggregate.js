...
function outputMonthlyParticipationSheet(monthlyData, hdcpData, startDate, endDate) {
  ...
  sheet.getRange(5, 1).setValue('年会員');
  sheet.getRange(5, 4, 1, months.length).setValues([months]);
...
function getHDCPData() {
  ...
  // A列=ID, N列(14列目)=新ハンディ, T列(20列目)=会員フラグ
  ...
  data.forEach(row => {
    ...
    // T列が厳密に1の場合のみ会員とする（数値・文字列どちらでも対応）
...
function outputToSheet(qualifiedMembers, guests, threshold, startDate, endDate) {
  ...
  // ========== スコア集計（試合数加味）シートを作成 ==========
  outputWeightedSheet(qualifiedMembers, threshold, startDate, endDate);
}
 ...
 * @param {Date} endDate - 終了日
 */
function outputWeightedSheet(qualifiedMembers, threshold, startDate, endDate) {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  ...
  // ヘッダー情報
  weightedSheet.getRange(1, 1).setValue('集計期間:');
  weightedSheet.getRange(1, 2).setValue(`${formatDate(startDate)} ～ ${formatDate(endDate)}`);
  weightedSheet.getRange(2, 1).setValue('参加日数閾値:');
  weightedSheet.getRange(2, 2).setValue(`${threshold}日以上`);
  weightedSheet.getRange(3, 1).setValue('※Net順に表示（参加日数閾値以上の会員のみ）');
  
  let currentRow = 5;
  
  // ヘッダー行
  const headers = ['順位', '会員ID', '会員名', '合計', '試合数', 'Gross', 'HDCP', 'Net', '参加日数', '備考'];
  ...
  // 書式設定
  formatWeightedSheet(weightedSheet, qualifiedMembers.length);
}

/**
 * スコア集計（試合数加味）シートの書式を設定
 * @param {Sheet} sheet - 対象シート
 * @param {number} dataCount - データ行数
 */
function formatWeightedSheet(sheet, dataCount) {
  // 列幅を調整
  ...
  sheet.setColumnWidth(9, 80);   // 参加日数
  sheet.setColumnWidth(10, 150); // 備考
  
  // 数値列を右揃え
  if (dataCount > 0) {
    sheet.getRange(6, 1, dataCount, 1).setHorizontalAlignment('center');  // 順位
    sheet.getRange(6, 2, dataCount, 1).setHorizontalAlignment('center');  // ID
    sheet.getRange(6, 4, dataCount, 6).setHorizontalAlignment('right');   // 数値列
  }
  
  // 罫線を設定
  if (dataCount > 0) {
    sheet.getRange(5, 1, dataCount + 1, 10).setBorder(
      true, true, true, true, true, true,
      'black', SpreadsheetApp.BorderStyle.SOLID
    );
  }
...
function getPlayerName(id) {
  ...
  if (masterSheet) {
    ...
    for (let i = 1; i < masterData.length; i++) {
      if (masterData[i][1] == id) {
        return masterData[i][0] || String(id);
      }
    }
  }
  
  // 会員マスターになければHDCPシートから取得
  const hdcpSheet = ss.getSheetByName('HDCP');
  if (hdcpSheet) {
    const hdcpData = hdcpSheet.getDataRange().getValues();
    // A列=ID, B列=名前
    for (let i = 1; i < hdcpData.length; i++) {
      if (hdcpData[i][0] == id) {
        return hdcpData[i][1] || String(id);
      }
    }
  }
...
function executeAggregation(startDate, endDate) {
  try {
    ...
    // 8. スコア集計シートに出力（qualifiedMembersをID順にソート済み、guestsもID順にソート済み）
    outputToSheet(qualifiedMembers, guests, threshold, startDate, endDate);
...
function outputWeightedSheet(qualifiedMembers, allMembers, guests, threshold, startDate, endDate) {
  ...
  // ========== 会員セクション（閾値以上） ==========
  // ヘッダー行
  const headers = ['順位', '会員ID', '会員名', '合計', '試合数', 'Gross', 'HDCP', 'Net', 'クロス順位', '参加日数', '備考'];
  weightedSheet.getRange(currentRow, 1, 1, headers.length).setValues([headers]);
  weightedSheet.getRange(currentRow, 1, 1, headers.length)
    .setBackground('#6AA84F')
    .setFontColor('white')
  ...
  weightedSheet.getRange(currentRow, 1, 1, headers.length).setValues([separatorRow]);
  weightedSheet.getRange(currentRow, 1, 1, headers.length)
  ...
  // 注釈
  weightedSheet.getRange(currentRow, 1).setValue(`日数 ${threshold}日以上をランキング対象とする`);
  weightedSheet.getRange(currentRow, 1).setFontWeight('bold');
  currentRow += 2;
  
  // ========== ゲストセクション ==========
  weightedSheet.getRange(currentRow, 1).setValue('ゲスト');
  weightedSheet.getRange(currentRow, 1).setFontWeight('bold');
  currentRow++;
  ...
  // 書式設定
  formatWeightedSheet(weightedSheet, qualifiedMembers.length, guests.length);
}
