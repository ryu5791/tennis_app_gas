...
function calculateHDCP() {
  ...
  try {
    ...
    // D3, G3 = それぞれ次の期に更新
    const oldD3 = String(hdcpSheet.getRange(3, 3).getValue());
    hdcpSheet.getRange(3, 3).setValue(getNextPeriod(oldD3)); // D3 = 次の期
    const oldG3 = String(hdcpSheet.getRange(3, 6).getValue());
    hdcpSheet.getRange(3, 6).setValue(getNextPeriod(oldG3)); // G3 = 次の期
